...
A=0.05
B=1000
gameArray=(21500499 21500511 21500527 21500543 21500559 21500575 21500601 21500622 21500011 21500021 21500046 21500063 21500078 21500094 21500106 21500130 21500141 21500160 21500176 21500191 21500203 21500219 21500227 21500262 21500288 21500313 21500334 21500367 21500384 21500405 21500424 21500438 21500453 21500466 21500473 21500291)
# agentArray=(ball shooter lastpasser shooterdefender lastpasserdefender)
agentArray=(shooter lastpasser)
...
for G in ${gameArray[@]}; do
    for agent in ${agentArray[@]}; do
        for D in 1.5 4; do
            for L in 5 8 10; do
                echo --------------------------
                echo Iteration $iter. Team $teamID Game $G, Agent $agent, Distance Threshold $D, Min Length $L...
                                
                # Get the start time for the current iteration
                iter_start_time=$(date +%s)
                
                if python -u nba_data_preprocess.py -d $downsamplingFactor -g $G -t $teamID -a $agent; then # 2>&1 >/dev/null; then
                    echo 'Successfully completed data preprocessing'
                fi

                echo Running Stat-dsm...
                outputDelta="$(python -u stat_dsm.py -alpha $A -d $D -l $L -i $B)"
                echo Finished running stat dsm and the output delta was $outputDelta
